...
function loadSelectedPresetTour() {
  ...
  if (!tour) {
    alert("Hãy chọn một tuyến mẫu, hoặc giữ chế độ Tự tạo Tour.");
    return;
  }
  ...
  if (available.length < 2) {
    alert("Tuyến mẫu hiện chưa có đủ ít nhất 2 địa điểm có tọa độ.");
    return;
  }
  ...
  if (notice) {
    notice.innerHTML = missing.length
      ...
      : `✅ Đã nạp ${available.length} địa điểm của tuyến mẫu.`;
  }
...
function showSelectedPresetInfo() {
  const tour = getSelectedPresetTour();
  if (!tour) {
    alert("Hãy chọn một tuyến mẫu để xem giới thiệu.");
    return;
  }

  const pointNames = (tour.points || []).map((ma, index) => {
    const feature = getFeatureByMa(ma);
    const name = feature?.properties?.TenDanhSach || `${ma} – dữ liệu đang cập nhật`;
    return `<li><b>${index + 1}.</b> ${escapeHtml(name)}</li>`;
  }).join("");

  showModal(`
    <h2>🛕 ${escapeHtml(tour.name || "Tuyến du lịch")}</h2>
    ...
    <div class="data-update-notice">
      Các địa điểm chưa nhập đủ dữ liệu vẫn được ghi nhận là “đang cập nhật” và không làm website bị lỗi.
    </div>
  `);
}
